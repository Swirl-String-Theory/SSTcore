...
**Contract:** treat `examples/example_*.ts` as the reference for request bodies and response shapes. Each example file matches one `src/*_node.cpp` binding module; backend handlers can wrap the same calls the examples use.
