...
IDEAL_DB_PATH = os.path.join(SCRIPT_DIR, "trefoil_closure/ideal.txt")
SST_CORE_CPP_PATH = os.path.join(SCRIPT_DIR, "sst_core.cpp")
SST_CORE_MODULE_NAME = "sst_core"
# Parent of trefoil_closure/ — SSTcore repo root (contains CMakeLists.txt, src/).
SST_PROJECT_ROOT = os.path.abspath(os.path.join(SCRIPT_DIR, ".."))
SST_CMAKE_BUILD_DIR = os.path.join(SST_PROJECT_ROOT, "cmake-build-py-sweep")
SST_CMAKE_CONFIG = "Release"
# Populated when --sstcore-cmake-build runs (or set manually); prepended before import.
SSTCORE_EXTRA_PATHS: List[str] = []
USE_SSTCORE_CMAKE_BUILD = False
...
# Which native module supplied calculate_bs_cutoff_energy_scan: "sstcore" | "sst_core" | "".
SST_CORE_SCAN_PROVIDER = ""

# Sweep defaults (preset "full"; "fast" overrides in main)
N_GEOM_LIST = [4000, 8000, 16000, 32000, 48000, 64000]
N_INT_LIST = [2000, 4000, 8000, 16000, 32000, 48000, 64000]
A_SCAN_COUNT = 24
...
def default_timestamped_exports_output_dir() -> str:
    """Unique run folder under ./exports/ (cwd): sst_knot_candidate_robustness_outputs_v10_3_<date_time>[_pid]."""
    stamp = time.strftime("%Y%m%d_%H%M%S")
    exports_root = os.path.join(os.getcwd(), "trefoil_closure/exports")
    base_name = f"sst_knot_candidate_robustness_outputs_v10_3_{stamp}"
    candidate = os.path.join(exports_root, base_name)
    if os.path.exists(candidate):
        base_name = f"sst_knot_candidate_robustness_outputs_v10_3_{stamp}_{os.getpid()}"
        candidate = os.path.join(exports_root, base_name)
    return os.path.abspath(candidate)


# Legacy placeholder until main() sets OUTPUT_DIR (timestamped exports default or --output-dir).
OUTPUT_DIR = os.path.abspath(os.path.join(SCRIPT_DIR, "sst_knot_candidate_robustness_outputs_v10_3"))
# Set while stdout/stderr tee to a console mirror log is active (see main()).
_ACTIVE_CONSOLE_LOG_PATH: Optional[str] = None
KNOT_ID = "3:1:1"
...
LAUNCHER_HEAVY_BATCH_KNOTS: List[str] = [
    "K11a367", "K11a247", "0:1:1", "3:1:1", "4:1:1", "5:1:1", "5:1:2","5:1:3",
    "6:1:1", "6:1:2", "6:1:3", "7:1:1", "7:1:2", "7:1:3","7:1:4", "8:1:1", "9:1:1", "9:1:2", "10:1:1", "0:1:2", "2:2:1", "l2a1", "l4a1", "l5a1", "l6a1", "l8a1"
]
LAUNCHER_HEAVY_BATCH_KNOTS_CSV = ",".join(LAUNCHER_HEAVY_BATCH_KNOTS)

LAUNCHER_HEAVY_EXACT_PAIRS: List[Tuple[int, int]] = [
    (24000, 24000), (32000, 32000), (40000, 40000), (48000, 48000), (56000, 56000),
    (64000, 64000), (72000, 72000), (80000, 80000), (88000, 88000), (96000, 96000),
]
...
LAUNCHER_DEFAULT_PARALLEL_SCOPE = "geom"
...
KNOT_PRESET_CATALOG: Dict[str, Dict[str, object]] = {
    "3:1:1": {"name": "Electron", "category": "lepton", "generation": "I", "k": 3.0, "g": 1, "G": 0, "pdg_mass_mev": 0.51099895},
    "5:1:1": {"name": "Muon", "category": "lepton", "generation": "II", "k": 5.0, "g": 2, "G": 1, "pdg_mass_mev": 105.6583745},
    "7:1:1": {"name": "Tau", "category": "lepton", "generation": "III", "k": 7.0, "g": 3, "G": 2, "pdg_mass_mev": 1776.86},
    "4:1:1": {"name": "Dark Matter", "category": "dark-sector", "generation": "-", "k": k_from_twist_alexander(1), "g": 1, "G": 1, "pdg_mass_mev": float("nan")},
    "5:1:2": {"name": "Up Quark", "category": "quark", "generation": "I", "k": k_from_twist_alexander(2), "g": 1, "G": 1, "pdg_mass_mev": 2.4},
    "6:1:1": {"name": "Down Quark", "category": "quark", "generation": "I", "k": k_from_twist_alexander(3), "g": 1, "G": 1, "pdg_mass_mev": 4.8},
    "7:1:2": {"name": "Strange Quark", "category": "quark", "generation": "II", "k": k_from_twist_alexander(4), "g": 1, "G": 1, "pdg_mass_mev": 104.0},
    "8:1:1": {"name": "Charm Quark", "category": "quark", "generation": "II", "k": k_from_twist_alexander(5), "g": 1, "G": 2, "pdg_mass_mev": 1270.0},
    "9:1:2": {"name": "Bottom Quark", "category": "quark", "generation": "III", "k": k_from_twist_alexander(6), "g": 1, "G": 2, "pdg_mass_mev": 4180.0},
    "10:1:1": {"name": "Top Quark", "category": "quark", "generation": "III", "k": k_from_twist_alexander(7), "g": 1, "G": 2, "pdg_mass_mev": 171200.0},
}

KNOT_PRESET_GROUPS: Dict[str, List[str]] = {
    "electron": ["3:1:1"],
    "leptons": ["3:1:1", "5:1:1", "7:1:1"],
    "quarks": ["5:1:2", "6:1:1", "7:1:2", "8:1:1", "9:1:2", "10:1:1"],
    "sm_fermions": ["3:1:1", "5:1:1", "7:1:1", "5:1:2", "6:1:1", "7:1:2", "8:1:1", "9:1:2", "10:1:1"],
    "twist_ladder": ["4:1:1", "5:1:2", "6:1:1", "7:1:2", "8:1:1", "9:1:2", "10:1:1"],
    "low_crossing_candidates": ["3:1:1", "4:1:1", "5:1:1", "5:1:2", "6:1:1", "7:1:1", "7:1:2"],
}

def normalize_knot_alias(token: str) -> str:
    token = token.strip()
    if not token:
        return token
    low = token.lower()
    alias_map = {
        "3_1": "3:1:1", "4_1": "4:1:1", "5_1": "5:1:1", "5_2": "5:1:2",
        "6_1": "6:1:1", "7_1": "7:1:1", "7_2": "7:1:2", "8_1": "8:1:1",
        "9_2": "9:1:2", "10_1": "10:1:1",
    }
    if low in alias_map:
        return alias_map[low]
    compact = low.replace(" ", "_")
    for knot_id, meta in KNOT_PRESET_CATALOG.items():
        if compact == str(meta.get("name", "")).lower().replace(" ", "_"):
            return knot_id
    return token

def expand_knot_tokens(tokens: Sequence[str]) -> List[str]:
    seen = set()
    out: List[str] = []
    for raw in tokens:
        tok = raw.strip()
        ...
        low = tok.lower()
        if low.startswith("@"):
            for item in KNOT_PRESET_GROUPS.get(low[1:], []):
                if item not in seen:
                    seen.add(item)
                    out.append(item)
            continue
        norm = normalize_knot_alias(tok)
        if norm not in seen:
            seen.add(norm)
            out.append(norm)
    return out

def get_knot_preset_metadata(knot_id: str) -> Dict[str, object]:
    return dict(KNOT_PRESET_CATALOG.get(knot_id, {}))


def parse_knot_list(text: str) -> List[str]:
    items = [tok.strip() for tok in text.replace(",", " ").split() if tok.strip()]
    return expand_knot_tokens(items)
...
def _prepend_import_paths(paths: Sequence[str]) -> None:
...
def _discover_sstcore_extension_dirs(build_dir: str, config: str) -> List[str]:
    """Directories that may contain sstcore.pyd / sstcore*.so after a CMake build."""
    roots: List[str] = []
    if not os.path.isdir(build_dir):
        return roots
    cfg_dir = os.path.join(build_dir, config)
...
def build_sstcore_via_cmake(
    project_root: str,
    build_dir: str,
    config: str = "Release",
) -> List[str]:
    """Configure and build the CMake `sstcore` pybind target. Returns dirs to prepend to sys.path."""
    cmake_exe = os.environ.get("CMAKE_EXE", "cmake")
    py_exe = sys.executable
    if not os.path.isfile(os.path.join(project_root, "CMakeLists.txt")):
        raise FileNotFoundError(f"No CMakeLists.txt under {project_root!r}")
    ...
    print("[META] CMake configure: {}".format(" ".join(configure)))
    subprocess.run(configure, check=True)
    build_cmd = [cmake_exe, "--build", build_dir, "--config", config, "--target", "sstcore"]
    print("[META] CMake build: {}".format(" ".join(build_cmd)))
    subprocess.run(build_cmd, check=True)
    return _discover_sstcore_extension_dirs(build_dir, config)
...
def resolve_bs_cutoff_scan_module(use_local: bool, auto_compile: bool) -> Tuple[Optional[object], bool]:
    ...
    _prepend_import_paths(SSTCORE_EXTRA_PATHS)

    mod, ok = _try_import_sstcore_scan()
    ...
    try:
        ...
        mod2 = importlib.import_module(SST_CORE_MODULE_NAME)
        sst_core = mod2
        _SST_CORE_AVAILABLE = True
        SST_CORE_SCAN_AVAILABLE = bool(hasattr(mod2, "calculate_bs_cutoff_energy_scan"))
        if SST_CORE_SCAN_AVAILABLE:
            SST_CORE_SCAN_PROVIDER = "sst_core"
        return mod2, SST_CORE_SCAN_AVAILABLE
    except Exception as exc:
...
def maybe_build_and_import_local_sst_core(use_local: bool, auto_compile: bool) -> Tuple[Optional[object], bool]:
    """Backward-compatible name for resolve_bs_cutoff_scan_module."""
    return resolve_bs_cutoff_scan_module(use_local, auto_compile)
...
def initialize_project_sources(knot_id: str, max_fourier_mode: Optional[int],
    ...
    resolve_bs_cutoff_scan_module(use_sst_core, auto_compile)
...
def summarize_project_sources() -> Dict[str, object]:
    ...
    return {
        ...
        "sst_core_scan_provider": SST_CORE_SCAN_PROVIDER,
        "sstcore_extra_paths": list(SSTCORE_EXTRA_PATHS),
    }
...
def write_best_estimate(rows: List[dict], outdir: str) -> None:
    ...
    with open(txt_path, "w", encoding="utf-8") as f:
        ...
        f.write(
            f"Local sst_core available    : {_SST_CORE_AVAILABLE}"
            f" (scan_provider={SST_CORE_SCAN_PROVIDER!r})\n"
        )
...
def run_v6_shiftfree_postcheck(rows: List[dict], outdir: str) -> List[dict]:
    ...
    try:
        ...
        csv_path = os.path.join(outdir, "shiftfree_postcheck_v10_2.csv")
        ...
        txt_path = os.path.join(outdir, "shiftfree_postcheck_v10_2.txt")
        with open(txt_path, "w", encoding="utf-8") as f:
        ...
        tex_path = os.path.join(outdir, "shiftfree_postcheck_v10_2.tex")
        with open(tex_path, "w", encoding="utf-8") as f:
...
def write_run_metadata(outdir: str, mode: str, backend_mode: str, backend_used: str,
    ...
    if _ACTIVE_CONSOLE_LOG_PATH:
        meta["console_log_path"] = _ACTIVE_CONSOLE_LOG_PATH
...
# -----------------------------------------------------------------------------
class _TeeTextIO:
    """Duplicate text writes to a primary stream (terminal) and a log file."""

    __slots__ = ("_primary", "_log_fp")
...
def prompt_cli_menu_v10() -> List[str]:
    ...
    output_dir = _prompt_text(
        "Output directory (blank = ./exports/sst_knot_candidate_robustness_outputs_v10_3_<timestamp>)",
        "",
    )
...
def _resolved_worker_count(task_count: int) -> int:
    if task_count <= 0:
        return 1
    if MAX_WORKERS and MAX_WORKERS > 0:
        return max(1, min(int(MAX_WORKERS), task_count))
    cpu_n = os.cpu_count() or 1
    return max(1, min(task_count, max(1, cpu_n - 1)))


def _should_parallelize_geom(task_count: int) -> bool:
...
def _apply_parallel_worker_settings(settings: Dict[str, object]) -> None:
    ...
    global SSTCORE_EXTRA_PATHS

    OUTPUT_DIR = str(settings["OUTPUT_DIR"])
    KNOT_ID = str(settings["KNOT_ID"])
    MAX_FOURIER_MODE = settings["MAX_FOURIER_MODE"]
    CONTACT_MODEL = str(settings["CONTACT_MODEL"])
    ROOT_SELECTION_MODE = str(settings["ROOT_SELECTION_MODE"])
    BRANCH_MODE = str(settings["BRANCH_MODE"])
    USE_LOCAL_IDEAL_DB = bool(settings["USE_LOCAL_IDEAL_DB"])
    USE_LOCAL_SST_CORE = bool(settings["USE_LOCAL_SST_CORE"])
    AUTO_COMPILE_SST_CORE = bool(settings["AUTO_COMPILE_SST_CORE"])
    ENABLE_TORCH_ACCEL = bool(settings["ENABLE_TORCH_ACCEL"])
    ...
    SSTCORE_EXTRA_PATHS = [str(x) for x in settings.get("SSTCORE_EXTRA_PATHS", [])]


def _compute_geom_config_result(payload: Dict[str, object]) -> Dict[str, object]:
    settings = payload["settings"]
    _apply_parallel_worker_settings(settings)
    mode = str(payload["mode"])
    N_geom = int(payload["N_geom"])
    N_int_target = int(payload["N_int_target"])
    R_ratio = payload.get("R_ratio")
    backend_used = str(payload["backend_used"])
    ...
    if mode == "torus" and R_ratio is not None:
        geom = build_torus_geometry(N_geom, N_int_target, float(R_ratio), 1.0)
        ideal_source_str = "Torus R_t/r_t={:.2f}".format(float(R_ratio))
    else:
    ...
    for fr in fit_results:
        ...
        lam_iter = sorted(LAMBDA_LIST) if LAMBDA_CONTINUATION_SORT else list(LAMBDA_LIST)
    ...
    summary = {
        ...
        "N_int_actual": int(geom.N_int_actual),
        "R_ratio": R_ratio if R_ratio is not None else np.nan,
        "L_dimless_full": float(geom.L_dimless_full),
        "d_min_full": float(geom.d_min_full),
        "t_geometry_s": float(t_geometry_s),
        "t_scan_s": float(t_scan_s),
        "t_fit_s": float(t_fit_s),
        "backend": backend_this,
        "best_fit_method": best["fit_method"] if best else "",
...
def _execute_geom_configs(mode: str, knot_id: str, output_dir: str, geom_configs: List[Tuple[int, int, Optional[float]]], backend_used: str) -> Tuple[List[dict], List[dict], List[dict], List[dict]]:
    ...
    with cf.ProcessPoolExecutor(max_workers=worker_count) as ex:
        fut_map = {ex.submit(_compute_geom_config_result, payload): (payload["N_geom"], payload["N_int_target"], payload.get("R_ratio")) for payload in payloads}
        done = 0
        for fut in cf.as_completed(fut_map):
            N_geom, N_int_target, R_ratio = fut_map[fut]
            res = fut.result()
...
def finalize_single_knot_sweep(args: argparse.Namespace, mode: str, knot_id: str, output_dir: str, run_start_wall: float, base_output_dir: str, rows: List[dict], plateau_diag_rows: List[dict], scan_curve_rows: List[dict], root_candidate_rows: List[dict]) -> Dict[str, object]:
    ...
    print(
        "[META] local_sst_core_loaded={} local_cpp_scan_available={} sst_core_scan_provider={!r}".format(
            _SST_CORE_AVAILABLE, SST_CORE_SCAN_AVAILABLE, SST_CORE_SCAN_PROVIDER
        )
    )
    ...
    if best:
        ...
        summary.update({
            ...
            "preset_name": preset_meta.get("name"), "category": preset_meta.get("category"), "generation": preset_meta.get("generation"),
            "k": preset_meta.get("k"), "g": preset_meta.get("g"), "G": preset_meta.get("G"), "pdg_mass_mev": preset_meta.get("pdg_mass_mev"),
        })
    return summary
...
def build_robustness_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--output-dir",
        default="",
        help="Output directory (default: ./exports/sst_knot_candidate_robustness_outputs_v10_3_<YYYYMMDD_HHMMSS> under cwd)",
    )
    ...
    parser.add_argument("--knot-preset", default="", help="Preset group selector, e.g. leptons, quarks, twist_ladder, low_crossing_candidates")
    parser.add_argument("--external-knotlib", choices=["auto", "none", "spherogram"], default="auto", help="Optional helper library for future knot metadata/invariants; current version uses built-in presets first")
    ...
    parser.add_argument(
        "--sst-project-root",
        default="",
        help="SSTcore repository root containing CMakeLists.txt (default: parent of trefoil_closure/)",
    )
    parser.add_argument(
        "--sst-cmake-build-dir",
        default="",
        help="CMake build directory (default: <sst-project-root>/cmake-build-py-sweep)",
    )
    parser.add_argument(
        "--sst-cmake-config",
        default="Release",
        help="MSVC multi-config build type when building sstcore (default: Release)",
    )
...
def run_robustness_sweep_with_args(args: argparse.Namespace) -> None:
    ...
    global SST_PROJECT_ROOT, SST_CMAKE_BUILD_DIR, SST_CMAKE_CONFIG, USE_SSTCORE_CMAKE_BUILD, SSTCORE_EXTRA_PATHS
    ...
    if args.sst_project_root:
        SST_PROJECT_ROOT = os.path.abspath(args.sst_project_root)
    if args.sst_cmake_build_dir:
        SST_CMAKE_BUILD_DIR = os.path.abspath(args.sst_cmake_build_dir)
    else:
        SST_CMAKE_BUILD_DIR = os.path.join(SST_PROJECT_ROOT, "cmake-build-py-sweep")
    SST_CMAKE_CONFIG = str(args.sst_cmake_config)
    if args.sstcore_cmake_build:
        USE_SSTCORE_CMAKE_BUILD = True
        if USE_LOCAL_SST_CORE:
            try:
                SSTCORE_EXTRA_PATHS = build_sstcore_via_cmake(
                    SST_PROJECT_ROOT, SST_CMAKE_BUILD_DIR, SST_CMAKE_CONFIG
                )
                print("[META] sstcore CMake prebuild OK; sys.path prepend dirs: {}".format(SSTCORE_EXTRA_PATHS))
            except Exception as exc:
                print("[WARN] sstcore CMake prebuild failed: {}".format(exc))
        else:
            print("[WARN] --sstcore-cmake-build ignored because --no-local-sst-core was set")
    if args.output_dir:
        OUTPUT_DIR = os.path.abspath(args.output_dir)
    else:
        OUTPUT_DIR = default_timestamped_exports_output_dir()
        print("[META] default output_dir (timestamped under ./exports/)={}".format(OUTPUT_DIR))
    ...
    if args.knot_preset:
        knot_tokens.extend(["@" + tok.strip().lstrip("@") for tok in args.knot_preset.replace(",", " ").split() if tok.strip()])
    if args.knot_list:
        knot_tokens.extend([tok.strip() for tok in args.knot_list.replace(",", " ").split() if tok.strip()])
    if not knot_tokens and args.knot_id:
        knot_tokens.append(args.knot_id)
    knot_ids = expand_knot_tokens(knot_tokens)
    if not knot_ids:
    ...
    _ACTIVE_CONSOLE_LOG_PATH = None
    if not args.no_console_log:
        try:
            if args.console_log:
                log_path = os.path.abspath(args.console_log)
            else:
                log_path = os.path.join(base_output_dir, "console_log_{}.log".format(time.strftime("%Y%m%d_%H%M%S")))
            log_parent = os.path.dirname(log_path)
            if log_parent:
                os.makedirs(log_parent, exist_ok=True)
            log_fp = open(log_path, "w", encoding="utf-8", newline="\n")
            _ACTIVE_CONSOLE_LOG_PATH = log_path
            torch_line = "torch=n/a"
            if _TORCH_AVAILABLE and torch is not None:
                try:
                    torch_line = "torch={} cuda_available={}".format(getattr(torch, "__version__", "?"), bool(torch.cuda.is_available()))
                except Exception:
                    torch_line = "torch={}".format(getattr(torch, "__version__", "?"))
            hdr = (
                "[LOG_HEADER] SST robustness sweep console mirror\n"
                "[LOG_HEADER] script_version={} started_wall={}\n"
                "[LOG_HEADER] log_path={}\n"
                "[LOG_HEADER] cwd={}\n"
                "[LOG_HEADER] argv={}\n"
                "[LOG_HEADER] python={} platform={}\n"
                "[LOG_HEADER] numpy={} pandas={} scipy={} {}\n"
                "[LOG_HEADER] output_base_dir={}\n"
                "[LOG_HEADER] knot_ids={}\n"
                "---\n"
            ).format(
                SCRIPT_VERSION,
                time.strftime("%Y-%m-%dT%H:%M:%S", time.localtime(run_start_wall)),
                log_path,
                os.getcwd(),
                sys.argv,
                sys.version.split()[0],
                sys.platform,
                getattr(np, "__version__", "?"),
                getattr(pd, "__version__", "?"),
                getattr(scipy, "__version__", "?"),
                torch_line,
                base_output_dir,
                knot_ids,
            )
            ...
            if log_fp is not None:
                try:
                    ...
                    pass
                log_fp = None
            print("[WARN] Could not open console log file: {}".format(exc), file=saved_stderr)

    try:
        if _ACTIVE_CONSOLE_LOG_PATH:
            print("[META] console_log_path={}".format(_ACTIVE_CONSOLE_LOG_PATH))
        ...
        if log_fp is not None:
            try:
                sys.stdout = saved_stdout
                sys.stderr = saved_stderr
                log_fp.write("\n[LOG_FOOTER] closed_wall={} total_script_s={:.3f}\n".format(time.strftime("%Y-%m-%dT%H:%M:%S", time.localtime(time.time())), time.perf_counter() - run_start))
            except Exception:
                pass
            try:
                log_fp.close()
...
def main() -> None:
    parser = build_robustness_arg_parser()
    argv_in = None
