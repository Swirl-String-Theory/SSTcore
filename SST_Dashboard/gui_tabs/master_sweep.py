...
def parse_ideal_database_embedded(embedded_name: str = "ideal.txt"):
    """
    Extract Knot IDs / crossings / components from embedded ideal.txt via C++ bindings.
    Returns list of dicts compatible with the sweep pipeline.
    """
    if hasattr(sstcore, "parse_embedded_ideal_txt"):
        blocks = sstcore.parse_embedded_ideal_txt(embedded_name)
    elif hasattr(sstcore, "load_embedded_ideal_text") and hasattr(sstcore, "parse_ideal_txt_from_string"):
        txt = sstcore.load_embedded_ideal_text(embedded_name)
        blocks = sstcore.parse_ideal_txt_from_string(txt)
    else:
        raise RuntimeError(
            "Embedded ideal parsing API not available. Need one of:\n"
            "  - parse_embedded_ideal_txt(name)\n"
            "  - load_embedded_ideal_text(name) + parse_ideal_txt_from_string(content)"
        )

    knots = []
    for ab in blocks:
        knot_id = getattr(ab, "id", "")
        try:
            crossings = int(str(knot_id).split(":")[0])
        except Exception:
            crossings = 0

        n_components = getattr(ab, "n", None)
        if n_components is None or n_components <= 0:
            comps = getattr(ab, "components", [])
            n_components = len(comps) if comps else 1
...
def parse_ideal_database(filepath: str):
    """Extracts all Knot IDs and component counts from the raw XML-like text (file fallback)."""
...
def _log(log_fn, msg):
    """Send message to callback or stdout."""
...
def run_master_sweep_embedded(
    output_csv: str,
    embedded_name: str = "ideal.txt",
    selected_ids=None,
    resolution_calibration: int = 1500,
    relax_iterations_calibration: int = 1500,
    resolution_batch: int = 1000,
    relax_iterations_batch: int = 1200,
    ...
    """
    Run the master sweep using embedded ideal.txt. GUI-friendly: accepts
    log_fn, progress_fn, cancel_check callbacks and optional selected_ids filter.
    """
    _log(log_fn, "\n=== SST MASTER SWEEP PIPELINE (Embedded ideal.txt) ===")

    knots_to_process = parse_ideal_database_embedded(embedded_name)
    _log(log_fn, f"[*] Found {len(knots_to_process)} unique topologies in embedded {embedded_name}.")

    if selected_ids:
        selected_set = set(selected_ids)
        knots_to_process = [k for k in knots_to_process if k["id"] in selected_set]
        _log(log_fn, f"[*] Filtered to {len(knots_to_process)} selected topologies.")

    if not knots_to_process:
        _log(log_fn, "[!] No knots to process.")
        return

    ELECTRON_MASS_MEV = 0.51099895
    _log(log_fn, "[*] Calibrating global Ether density via the Electron (3:1:1)...")

    try:
        e_particle = sstcore.ParticleEvaluator("3:1:1", resolution=resolution_calibration)
        e_particle.relax(iterations=relax_iterations_calibration, timestep=timestep)
        L_tot_e = e_particle.get_dimless_ropelength(stretch_lambda=1.0)
        k_e = k_from_crossings(3)
        M0 = ELECTRON_MASS_MEV / (
            2.0 * (k_e**-1.5) * (phi0**-1) * (1.0**(-1.0/phi0)) * L_tot_e
        )
        _log(log_fn, f"[+] Calibration complete. M0 = {M0:.6f} MeV")
    except Exception as e:
        _log(log_fn, f"[!] Calibration failed: {e}. Aborting.")
        raise

    _log(log_fn, f"[*] Commencing batch processing. Results will be saved to {output_csv}")

    with open(output_csv, mode='w', newline='', encoding='utf-8') as csv_file:
        fieldnames = [
            'Knot_ID', 'Crossings', 'Components', 'Chi_Spin', 'Lambda', 'L_tot',
            'M_abinitio_core_MeV', 'delta_kernel_vs_abinitio_core',
            'Mass_G0_MeV', 'Match_G0', 'delta_G0_vs_abinitio_core', 'ratio_G0_to_abinitio_core',
            'Mass_G1_MeV', 'Match_G1', 'delta_G1_vs_abinitio_core', 'ratio_G1_to_abinitio_core',
            'Mass_G2_MeV', 'Match_G2', 'delta_G2_vs_abinitio_core', 'ratio_G2_to_abinitio_core',
        ...
        total = len(knots_to_process)

        for idx, knot in enumerate(knots_to_process):
            if cancel_check is not None and cancel_check():
                _log(log_fn, "[!] Sweep cancelled by user.")
                break
            ...
            if progress_fn is not None:
                progress_fn(idx, total, k_id)

            _log(log_fn, f"    [{idx+1}/{total}] Simulating {k_id}...")
            ...
            try:
                particle = sstcore.ParticleEvaluator(k_id, resolution=resolution_batch)
                particle.relax(iterations=relax_iterations_batch, timestep=timestep)
                L_tot = particle.get_dimless_ropelength(stretch_lambda=lambda_val)
                ...
                _log(log_fn, f"       DONE (L_tot={L_tot:.3f}, chi={chi_val}, lambda={lambda_val:.5f})")
            except Exception as e:
            ...
            writer.writerow({
                ...
                'ratio_G3_to_abinitio_core': round(ratio_G[3], 6) if ratio_G[3] is not None else '',
                'Best_G': diagnostics["Best_G"],
                'Best_Mass_MeV': round(diagnostics["Best_Mass_MeV"], 3) if diagnostics["Best_Mass_MeV"] is not None else '',
                'Best_Match_Label': diagnostics["Best_Match_Label"] or '',
                'Best_delta_vs_abinitio_core': round(diagnostics["Best_delta_vs_abinitio_core"], 6) if diagnostics["Best_delta_vs_abinitio_core"] is not None else '',
                'Best_ratio_to_abinitio_core': round(diagnostics["Best_ratio_to_abinitio_core"], 6) if diagnostics["Best_ratio_to_abinitio_core"] is not None else '',
            })
...
if __name__ == "__main__":
    ...
    OUTPUT_FILE = str(out_dir / f"sst_master_sweep_results_{ts}.csv")
    LOG_FILE = str(out_dir / f"sst_master_sweep_log_{ts}.txt")

    # Use embedded ideal.txt by default; set SST_USE_FILE_DB=1 to use file-based parser
    use_embedded = os.environ.get("SST_USE_FILE_DB", "").strip() != "1"
        ...
        with open(LOG_FILE, "w", encoding="utf-8") as log_file:
            ...
            try:
                run_master_sweep_embedded(
                    ...
                    selected_ids=None,
                    log_fn=log,
                    progress_fn=lambda d, t, kid: log(f"  [{d+1}/{t}] {kid}"),
                )
