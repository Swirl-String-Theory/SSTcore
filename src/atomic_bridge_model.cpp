...
double eta0(double v_swirl, double c) { return SSTCanonicalConstants::eta0(v_swirl, c); }
...
double clock_branch_factor(double sigma, double eta0_val, double gamma) {
    return 1.0 + sigma * eta0_val * gamma;
}

double scalar_clock_potential_linear(double sigma, double E_R, double eta0_val, double gamma) {
    return -2.0 * sigma * E_R * eta0_val * gamma;
}

double orbit_coupling_energy(double sigma, double E_R, double eta0_val, double anticommutator_value) {
    return 0.5 * E_R * eta0_val * sigma * anticommutator_value;
}

double frozen_linear_interaction(double sigma, double E_R, double eta0_val, double gamma, double anticommutator_value) {
    return scalar_clock_potential_linear(sigma, E_R, eta0_val, gamma) + orbit_coupling_energy(sigma, E_R, eta0_val, anticommutator_value);
}

double pauli_barrier_scale(double rho_f, double Gamma0, double L, double a_cut, double shape_factor) {
    ...
    if (shape_factor < 0.0) throw std::invalid_argument("shape_factor must be non-negative.");
